...
def _scrape_sync(overstat_url: str) -> dict:
    """Run Playwright synchronously in a thread."""
    ...
    try:
        with sync_playwright() as p:
            ...
            print(f"[SCRAPER] Got {len(lines)} lines. First 10: {lines[:10]}")
            ...
            print(f"[SCRAPER] Done: user={username} dmg={avg_dmg} "
                  f"kd={kd} ast={assists} kills={total_kills} "
                  f"srt={survival_time} legend={most_played_legend}")
